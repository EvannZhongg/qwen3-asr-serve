...
```bash
# 1. 安装依赖 + 下载模型（约 6 GB，模型通过 ModelScope 写入 ./models）
./install.sh
...
# 3. 调用
curl -F file=@some.wav http://localhost:9123/v1/audio/transcriptions
```

默认监听 `0.0.0.0:9123`。通过 `.env`（复制 `.env.example` 修改）或环境变量配置。
...
```bash
# 最简：multipart 上传
curl -F file=@a.wav http://localhost:9123/v1/audio/transcriptions

# 带字级时间戳，OpenAI verbose_json 形态
curl -F file=@a.wav \
     -F response_format=verbose_json \
     -F 'timestamp_granularities[]=word' \
     http://localhost:9123/v1/audio/transcriptions
...
curl -F 'audio_files=@a.wav' -F 'audio_files=@b.wav' \
     ...
     http://localhost:9123/v1/audio/transcriptions/batch
...
curl -F file=@a.wav -F text="刚收到快递" -F language=zh \
     http://localhost:9123/v1/audio/forced_alignment
```
...
| `ENABLE_AUDIO_DURATION_METRICS` | `true` | 是否为 `qwen_asr_audio_seconds_total` 探测音频时长；关闭可减少大 batch 下的 `soundfile.info()` 开销 |
...
│       ├── install_runtime_scripts.sh    构建期安装主入口；conda hook 默认注释，便于测试 CMD 是否被平台覆盖
│       ├── 99-qwen3-asr-autostart.sh     历史参考：conda activate.d hook，可直接通过 ./run.sh -d 自启动
